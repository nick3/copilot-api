...
describe("resolveWebSearchRoute", () => {
  ...
  it("routes a provider/model alias to provider passthrough", () => {
    const route = resolveWebSearchRoute(makePayload(), {
      ...opts,
      isProviderAvailable: (provider) => provider === "anthropic",
  ...
  it("does not route missing provider/model fallback values to provider passthrough", () => {
    const route = resolveWebSearchRoute(makePayload(), {
      ...opts,
      isProviderAvailable: () => false,
      webSearchModel: "missing/gpt-search",
    })
    expect(route).toEqual({
      kind: "responses",
      model: "missing/gpt-search",
    })
  })
...
describe("handleWebSearchViaResponses", () => {
  it("switches model, runs Responses web_search, and reconstructs blocks", async () => {
    ...
    const types = response.content.map((b) => b.type)
  ...
  it("throws instead of reconstructing a failed Responses result", async () => {
    let usageRecorded = false
    webSearchFlowDependencies.createResponses = () =>
      Promise.resolve(
        makeResponsesResult({
          error: { message: "upstream search failed" },
          output: [],
          status: "failed",
        }),
      )
    ...
    await expectRejectsWithMessage(
      () => handleWebSearchViaResponses(c, makePayload(), baseOptions),
      "upstream search failed",
    )
    expect(usageRecorded).toBe(false)
  })

  it("throws instead of reconstructing an incomplete Responses stream", async () => {
    const result = makeResponsesResult({
      incomplete_details: { reason: "max_output_tokens" },
      output: [],
      status: "incomplete",
    })
    async function* makeIncompleteStream() {
      await Promise.resolve()
      yield {
    ...
    webSearchFlowDependencies.createResponses = () =>
      Promise.resolve(makeIncompleteStream())
    webSearchFlowDependencies.createUsageRecorder = () => () => {}
